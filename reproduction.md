The chromium code below is used to create an ID3D11Texture that can be used cross-process.
Create a minimal version of this class that produces only `DXGI_FORMAT_R8G8B8A8_UNORM` textures. We want to keep the same approach of maintaining a private ID3D11Device that produces and owns these textures. It's important for the reproduction that the device inside the memory buffer factory is only used to create the textures and nothing else. Use a separate device backed by the same high performance adapter to render a unique solid color to each texture.

The test should create 3840x2160 sized textures. Use CMake with the Ninja generator and `out/dev` for the build directory. Write the code using C++20 and with only Windows and C++ stdlib headers. See @with-devenv.ps1 for a starter script to use for building.
...
void GpuMemoryBufferFactoryDXGI::DestroyGpuMemoryBuffer(
  ...
  if (d3d11_device_) {
    ...
    LOG(INFO) << "DestroyGpuMemoryBuffer called with id: " << id.id
              << ", client_id: " << client_id
              << " calling flush on device context.";
    d3d11_device_context->Flush();
  } else {
    LOG(INFO) << "DestroyGpuMemoryBuffer called with id: " << id.id
              << ", client_id: " << client_id
              << " but D3D11 device is not initialized.";
  }
}
